...
export async function setupAIFeaturesTables(): Promise<void> {
  try {
    ...
    await fetch(`${BACKEND_URL}/api/v2/database/execute-raw`, {
      method: 'POST',
    ...
    const templateCountResponse = await fetch(`${BACKEND_URL}/api/v2/database/query-raw`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        query: 'SELECT COUNT(*) as count FROM "ContentTemplate"'
      })
    });
    const templateCount = templateCountResponse.ok ? await templateCountResponse.json() : [{ count: 0 }];
          ...
          body: JSON.stringify({
            query: `
              ...
              VALUES (
                ...
                ${template.industry ? `'${template.industry}'` : 'NULL'},
                '${template.category}',
...
export async function checkAIFeaturesTables(): Promise<boolean> {
  try {
    ...
    const engagementTimeTable = response.ok ? await response.json() : [{ exists: false }];
