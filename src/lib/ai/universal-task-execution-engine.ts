/**
 * Universal Task Execution Engine - Frontend Stub
 * ================================================
 *
 * This is a TEMPORARY stub file to prevent build breakage.
 * The actual implementation should exist in the backend.
 *
 * TODO: Migrate to use backend API
 */

const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3006';

class UniversalTaskExecutionEngine {
  constructor() {
    console.warn('⚠️  Using stub UniversalTaskExecutionEngine. Migrate to backend API.');
  }

  async execute(task: any): Promise<any> {
    throw new Error('Method not implemented - use backend API');
  }

  async getStatus(taskId: string): Promise<any> {
    throw new Error('Method not implemented - use backend API');
  }
}

export const universalTaskExecutionEngine = new UniversalTaskExecutionEngine();
export default universalTaskExecutionEngine;
