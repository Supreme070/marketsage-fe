...
export default function LeadPulseAnalytics() {
  ...
  useEffect(() => {
    ...
    const streamAIVisitorData = async () => {
      try {
        // Get database visitors
        const visitors = await getActiveVisitors('1h');
        
        // Also check simulator for real-time data
        let simulatorData = null;
        try {
          const simulatorResponse = await fetch('/api/leadpulse/simulator?action=status');
          simulatorData = await simulatorResponse.json();
        } catch (error) {
          console.log('Could not fetch simulator data:', error);
        }
        ...
        // Add simulator active visitors count to the display
        if (simulatorData && simulatorData.isRunning) {
          console.log(`Simulator active with ${simulatorData.activeVisitors} visitors`);
          // You can add simulator visitors to the display here
        }
