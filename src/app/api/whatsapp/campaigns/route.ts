...
//  Schema for WhatsApp campaign validation with compliance checks
const campaignSchema = z.object({
  ...
  from: z.string().min(1, "From number is required").refine(
    ...
    {
      message: "Invalid sender phone number format. Must be a valid international format (e.g., +234XXXXXXXXXX)"
    }
  ),
  templateId: z.string().optional(),
  content: z.string().optional().refine(
    (content) => !content || content.length <= 4096,
    {
      message: "Message content cannot exceed 4096 characters (WhatsApp limit)"
    }
  ),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [campaigns, totalCount] = await Promise.all([
      ...
      prisma.whatsAppCampaign.count({ where: whereClause })
    ]);
  ...
  try {
    ...
    if (campaignData.enableBirthdayTargeting) {
      try {
        ...
        console.log(`Birthday detection triggered for WhatsApp campaign: ${newCampaign.id}`);
      } catch (birthdayError) {
    ...
    return NextResponse.json({
      ...
      birthdayIntegration: campaignData.enableBirthdayTargeting ? {
        ...
        message: "Birthday detection has been triggered for this WhatsApp campaign"
      } : null
    }, { status: 201 });
  } catch (error: any) {
