...
// Schema for personalization preview request
...
// POST endpoint to preview personalized SMS content
  ...
  try {
    ...
    return NextResponse.json({
      ...
      analysis: {
        ...
        smsSegments: Math.ceil(personalizedContent.length / 160), // SMS segment calculation
        insights
      },
    ...
    return handleApiError(error, "/api/sms/personalization/preview/route.ts");
  }
}

// Enhanced personalization function (reused from campaign sending)
...
function generatePersonalizationInsights(original: string, contact: any, personalized: string): string[] {
  ...
  if (personalized.length > 160) {
    const segments = Math.ceil(personalized.length / 160);
    insights.push(`📱 Message will be sent as ${segments} SMS segments (${personalized.length} characters).`);
  }
